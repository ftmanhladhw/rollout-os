...
- ✅ **Command Center — fully live** (`src/components/command-center/`): vital-signs strip (manual Health · mean workstream Progress · Readiness roll-up · Go Live countdown) + five live sections (Today's Priorities = nearest dated open tasks, Blockers = status-blocked tasks/milestones/issues, Upcoming Milestones with blocked/overdue/due-soon signals, My Work = my tasks + my risks, Recent Activity = activity log). `placeholder-data.ts` deleted. Shared metric helpers live in `src/lib/rollout-metrics.ts` (one definition for Command Center, Reports, Timeline). Semantic status tokens (`--status-good/warning/critical`) in globals.css — color never carries meaning without a text label (docs/07 ch.6).
...
- ✅ **Timeline module** (5 of 7; `src/app/(app)/timeline/`): read-only projection — phases in order, each grouping its milestones + meetings (dated first), unphased items trailing, Go Live marker with countdown. `phaseId` is now assignable on milestone/meeting drawers (validated as a live phase of the caller's rollout). Pure grouping/countdown helpers unit-tested (`timeline/lib.ts`).
- ✅ **Reports module** (6 of 7; `src/app/(app)/reports/`): four generated reports — Executive (vitals + delivery + top risks + recent decisions) · Weekly (7-day window: done/new/blocked/upcoming) · Risk (severity-ranked, probability×impact) · Readiness (dimension roll-up). Pure aggregation helpers unit-tested (`reports/lib.ts`). First page behind `requireCan('reports:generate')` — engineering/client land on `/unauthorized`. Health/readiness/risk-level labels joined the terminology layer.
- ✅ **Administration module** (7 of 7; `src/app/(app)/settings/`): Members (role + experience-profile editing, removal — `org:manage`, with last-org-admin protection) · Rollout settings (name/status/**manual health**/priority/go-live + per-dimension **manual readiness** — `rollout:manage`). Tabs gate independently; holders of neither action land on `/unauthorized`. Teams, templates (docs/09 scope-out), and **invites** are later slices — today a second user can only join by signup, which creates their own org (invite flow is the known gap).
- ✅ **All seven modules are live, and the Command Center is fully wired to live data.** Next: doc 08 and the platform chores below.
